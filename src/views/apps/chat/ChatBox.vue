...
const defaultUserId = ref('company');
...
const getInitials = (name) => {
    if (typeof name === 'string' && name.trim() !== '') {
        ...
        // Handle the case where name is not a valid string
        return '';
    }
};
</script>
    ...
    <div class="flex flex-column h-full">
        <div class="flex align-items-center border-bottom-1 surface-border p-3 lg:p-6">
            <div class="relative flex align-items-center mr-3">
                <Avatar :label="getInitials(user.name)" size="xlarge" shape="circle" />
            ...
            <div class="mr-2">
                ...
                <span class="text-700">Last active {{ user.lastSeen }} ago</span>
            </div>
        ...
        <div class="user-message-container p-3 md:px-4 lg:px-6 lg:py-4 mt-2 overflow-y-auto" style="max-height: 53vh">
            <div v-for="message in user.messages" :key="message">
                <div v-if="message.owner !== defaultUserId" class="grid grid-nogutter mb-4">
                    <div class="mr-3 mt-1">
                        <Avatar :label="getInitials(user.name)" size="large" shape="circle" />
                    </div>
                    <div class="col mt-3">

...
const submit = async () => {
    ...
    loading.value = true;
    try {
        const tableData = await HTTP.post('cash-reports/adminReport', {
            date: dates.value,
            admin: admin.value
        });
...
const initFilters1 = () => {
    filters1.value = {
        ...
        admin: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.CONTAINS }] },
    ...
    <div class="">
        ...
        <div class="card">
            ...
            <DataTable
                ...
                :loading="loading1"
                :filters="filters1"
                :globalFilterFields="['admin', 'bets', 'slipCount', 'payOut', 'unclaimed', 'ggr', 'rtp', 'net']"
                ref="dt"
                showGridlines
            >
                <template #header>

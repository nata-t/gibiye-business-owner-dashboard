...
const columns = ref([
    ...
    { field: 'scheduleTime', header: 'Appointment Time' },
...
const dashboardData = async () => {
    try {
        const tableData = await HTTP.post('manage_bookings/getBusinessAppointment');
...
const approve = async (data) => {
    try {
        const response = await HTTP.post('manage_bookings/approveBooking', { bookingId: data.id });
        if (response.status == 200) {
            await dashboardData();
            toast.add({ severity: 'success', summary: 'Success Message', detail: 'Booking Approved Successfully', life: 3000, id: 'success' });
            setTimeout(tryAgain, 10000);
        }
    } catch (error) {
        let errorMessage = 'Unable to approve booking';
        if (error.response) {
            errorMessage = error.response.data.message || 'Server error occurred';
        } else if (error.request) {
            errorMessage = 'No response from server';
    ...
    <div class="card mt-6">
        ...
        >
            ...
            <Column field="action" header="Action">
                <template #body="{ data }">
                    <Button label="approve" outlined @click="approve(data)" text />
                </template>
            </Column>
        </DataTable>
    </div>
